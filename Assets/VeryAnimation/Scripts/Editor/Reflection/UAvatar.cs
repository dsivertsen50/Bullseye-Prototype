...
namespace VeryAnimation
{
    internal sealed class UAvatar
    {
        ...
        public MuscleLimit GetMuscleLimit(Avatar avatar, HumanBodyBones humanoidIndex)
        {
            ...
            if (pAxesArray == null || !pAxesArray.isArray || axesId < 0 || axesId >= pAxesArray.arraySize)
                return null;
            var pAxes = pAxesArray.GetArrayElementAtIndex(axesId);
            if (pAxes == null)
                return null;
        ...
        public Dictionary<string, Transform> GetDefaultPose(Avatar avatar)
        {
            ...
            {
                var count = Mathf.Min(pSkeletonPose.arraySize, pID.arraySize);
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (!idPath.TryGetValue(pID.GetArrayElementAtIndex(i).longValue, out string path))
                        continue;
                    list.TryAdd(path, new Transform()
                    {
                        position = position,
                        rotation = rotation,
                        scale = scale,
