...
{
    [Serializable]
    internal sealed class VeryAnimationWindow : EditorWindow
    {
        public const string Version = "1.4.1";

        private static readonly Color HandleColorActive = new(1f, 1f, 1f, 0.5f);
        private static readonly Color HandleColorInactive = new(1f, 1f, 1f, 0.2f);

        private const string PrefKey_Animation = "VeryAnimation_Main_Animation";
        private const string PrefKey_Tools = "VeryAnimation_Main_Tools";
        private const string PrefKey_Settings = "VeryAnimation_Main_Settings";
        private const string PrefKey_Help = "VeryAnimation_Main_Help";
        private const string PrefKey_Preview = "VeryAnimation_Main_Preview";

        [MenuItem("Window/Very Animation/Main", priority = 1000)]
        ...
        [MenuItem("Window/Very Animation/Tutorial", priority = 1100)]
        public static void DisplayTutorialDialog()
        {
        ...
        private VeryAnimation.ErrorCode beforeErrorCode;
        ...
        private bool readyWarningsUpdate = true;
        private bool readyTDOFWarning;
        private readonly List<(HumanBodyBones parent, HumanBodyBones bone)> readyTDOFWarningBones = new();
        private readonly List<(string meshName, MeshTopology topology)> readyMeshWarnings = new();

        #endregion

        #region ClipSelector
#if UNITY_6000_4_OR_NEWER
        internal class SelectorTreeView : TreeView<EntityId>
#elif UNITY_6000_2_OR_NEWER
        internal class SelectorTreeView : TreeView<int>
#else
        internal class SelectorTreeView : TreeView
#endif
        {
...
#if UNITY_6000_4_OR_NEWER
            public SelectorTreeView(TreeViewState<EntityId> state) : base(state)
#elif UNITY_6000_2_OR_NEWER
            ...
            {
                var root = new TreeViewItem<EntityId>(EntityId.None, -1, "Root")
                {
                    children = new List<TreeViewItem<EntityId>>()
                };
                if (VAW.GameObject != null && AnimationClips != null)
                {
                    foreach (var clip in AnimationClips)
                    {
                        root.children.Add(new TreeViewItem<EntityId>(clip.GetEntityId(), 0, clip.name));
                    }
                }
                return root;
            }
            ...
            public void UpdateSelectedIds()
            {
                ...
                {
                    var index = AnimationClips.IndexOf(VAW.VA.UAw.GetSelectionAnimationClip());
                    if (index >= 0 && AnimationClips[index] != null)
                    {
#if UNITY_6000_4_OR_NEWER
                        state.selectedIDs.Add(AnimationClips[index].GetEntityId());
#else
...
#if UNITY_6000_4_OR_NEWER
        public TreeViewState<EntityId> ClipSelectorTreeState { get; private set; }
#elif UNITY_6000_2_OR_NEWER
...
#if UNITY_6000_4_OR_NEWER
            ClipSelectorTreeState = new TreeViewState<EntityId>();
#elif UNITY_6000_2_OR_NEWER
        ...
        private struct SelectionRect
        {
            public void Reset()
            {
                ...
                End = Vector2.zero;
                Distance = 0f;
                calcList ??= new List<GameObject>();
                calcList.Clear();
                virtualCalcList ??= new List<HumanBodyBones>();
                virtualCalcList.Clear();
                animatorIKCalcList ??= new List<AnimatorIKCore.IKTarget>();
                animatorIKCalcList.Clear();
                originalIKCalcList ??= new List<int>();
                originalIKCalcList.Clear();
...
#pragma warning disable IDE0051
            static string[] OnWillSaveAssets(string[] paths)
            {
                if (enable)
                {
                    ReleaseAllInitialized("OnWillSaveAssets");
                }
                return paths;
            }
#pragma warning restore IDE0051
            ...
            internal sealed class PauseScope : IDisposable
            {
        ...
        internal class CustomCompilationListener
        {
            ...
            {
                ReleaseAllInitialized("AssemblyReloadEvents.beforeAssemblyReload");
            }
        ...
        {
            ReleaseAllInitialized("InitializeOnLoadMethod");
        }

        private static void ReleaseAllInitialized(string reason)
        {
            foreach (var w in Resources.FindObjectsOfTypeAll<VeryAnimationWindow>())
            {
                if (w.Initialized)
                {
                    w.Release();
                    Debug.Log($"<color=blue>[Very Animation]</color>Editing ended : {reason}");
                }
            }
        }
        ...
        static void CloseOtherWindows()
        {
            ...
            static void DefensiveCloseWindow(Type type)
            {
                while (true)
                {
                    var w = EditorCommon.FindResourceFirst<EditorWindow>(x => type.IsInstanceOfType(x));
                    if (w == null)
                        break;
                    try
                    {
            ...
            DefensiveCloseWindow(typeof(VeryAnimationRangePinningBaseWindow));
        }

        private void OnPlayModeStateChanged(PlayModeStateChange mode)
        {
            ReleaseAllInitialized("OnPlayModeStateChanged");
        }
        private void OnPauseStateChanged(PauseState mode)
        {
            ReleaseAllInitialized("OnPauseStateChanged");
        }
        #endregion
        ...
        class GUIStyles
        {
            public readonly GUISkin guiSkinSceneWindow;
            public readonly GUIStyle guiStyleSceneWindow;
            public readonly GUIStyle guiStyleSkinBox;
            public readonly GUIStyle guiStyleBoldButton;
            public readonly GUIStyle guiStyleCircleButton;
            public readonly GUIStyle guiStyleCircle3Button;
            public readonly GUIStyle guiStyleDiamondButton;
            public readonly GUIStyle guiStyleCircleDotButton;
            public readonly GUIStyle guiStyleCenterAlignLabel;
            public readonly GUIStyle guiStyleCenterAlignYellowLabel;
            public readonly GUIStyle guiStyleBoldFoldout;
            public readonly GUIStyle guiStyleDropDown;
            public readonly GUIStyle guiStyleToolbarBoldButton;
            public readonly GUIStyle guiStyleAnimationRowEvenStyle;
            public readonly GUIStyle guiStyleAnimationRowOddStyle;
            public readonly GUIStyle guiStyleMiddleRightMiniLabel;
            public readonly GUIStyle guiStyleMiddleLeftGreyMiniLabel;
            public readonly GUIStyle guiStyleMiddleRightGreyMiniLabel;
            public readonly GUIStyle guiStyleMirrorButton;
            public readonly GUIStyle guiStyleLockToggle;
            public readonly GUIStyle guiStyleIconButton;
            public readonly GUIStyle guiStyleIconActiveButton;
            public readonly GUIContent[] guiContentMoveRotateTools;
            public readonly GUIContent guiContentTutorialVideoPlaylist;

            private static GUIStyle CreateTextureButtonStyle(Texture2D normalTex, Texture2D activeTex)
            {
            ...
            public GUIStyles(Texture2D circleNormal, Texture2D circleActive,
                             Texture2D circle3Normal, Texture2D circle3Active,
                             Texture2D diamondNormal, Texture2D diamondActive,
                             Texture2D circleDotNormal, Texture2D circleDotActive,
                             Texture2D mirrorTex)
            {
                ...
                guiStyleCircleButton = CreateTextureButtonStyle(circleNormal, circleActive);
                guiStyleCircle3Button = CreateTextureButtonStyle(circle3Normal, circle3Active);
                guiStyleDiamondButton = CreateTextureButtonStyle(diamondNormal, diamondActive);
                guiStyleCircleDotButton = CreateTextureButtonStyle(circleDotNormal, circleDotActive);
                ...
                guiContentTutorialVideoPlaylist = new GUIContent("Tutorial Video Playlist", "YouTube");
            }
        }
        ...
        private GUIStyles Styles
        {
            get
            {
                guiStyles ??= new GUIStyles(
                    circleNormalTex, circleActiveTex,
                    circle3NormalTex, circle3ActiveTex,
                    diamondNormalTex, diamondActiveTex,
                    circleDotNormalTex, circleDotActiveTex,
                    MirrorTex);
                return guiStyles;
            }
        }

        public bool IsGuiStyleReady => guiStyles != null;

        public GUISkin GuiSkinSceneWindow => Styles.guiSkinSceneWindow;
        public GUIStyle GuiStyleSceneWindow => Styles.guiStyleSceneWindow;
        public GUIStyle GuiStyleSkinBox => Styles.guiStyleSkinBox;
        public GUIStyle GuiStyleBoldButton => Styles.guiStyleBoldButton;
        public GUIStyle GuiStyleCircleButton => Styles.guiStyleCircleButton;
        public GUIStyle GuiStyleCircle3Button => Styles.guiStyleCircle3Button;
        public GUIStyle GuiStyleDiamondButton => Styles.guiStyleDiamondButton;
        public GUIStyle GuiStyleCircleDotButton => Styles.guiStyleCircleDotButton;
        public GUIStyle GuiStyleCenterAlignLabel => Styles.guiStyleCenterAlignLabel;
        public GUIStyle GuiStyleCenterAlignYellowLabel => Styles.guiStyleCenterAlignYellowLabel;
        public GUIStyle GuiStyleBoldFoldout => Styles.guiStyleBoldFoldout;
        public GUIStyle GuiStyleDropDown => Styles.guiStyleDropDown;
        public GUIStyle GuiStyleToolbarBoldButton => Styles.guiStyleToolbarBoldButton;
        public GUIStyle GuiStyleAnimationRowEvenStyle => Styles.guiStyleAnimationRowEvenStyle;
        public GUIStyle GuiStyleAnimationRowOddStyle => Styles.guiStyleAnimationRowOddStyle;
        public GUIStyle GuiStyleMiddleRightMiniLabel => Styles.guiStyleMiddleRightMiniLabel;
        public GUIStyle GuiStyleMiddleLeftGreyMiniLabel => Styles.guiStyleMiddleLeftGreyMiniLabel;
        public GUIStyle GuiStyleMiddleRightGreyMiniLabel => Styles.guiStyleMiddleRightGreyMiniLabel;
        public GUIStyle GuiStyleMirrorButton => Styles.guiStyleMirrorButton;
        public GUIStyle GuiStyleLockToggle => Styles.guiStyleLockToggle;
        public GUIStyle GuiStyleIconButton => Styles.guiStyleIconButton;
        public GUIStyle GuiStyleIconActiveButton => Styles.guiStyleIconActiveButton;
        public GUIContent[] GuiContentMoveRotateTools => Styles.guiContentMoveRotateTools;
        public GUIContent GuiContentTutorialVideoPlaylist => Styles.guiContentTutorialVideoPlaylist;
        #endregion
        ...
        private void OnGUI()
        {
...
#if Enable_Profiler
            Profiler.BeginSample($"****VeryAnimationWindow.OnGUI {Event.current}");
#endif
            ...
            else if (!VA.IsEdit)
            {
                        ...
                        if (VA.UAw.GetLinkedWithTimeline() || EditorApplication.isPlaying)
                        {
                            ...
                            if (PlayingAnimationInfos != null)
                            {
                                for (int i = 0; i < PlayingAnimationInfos.Length; i++)
                                {
                                    ...
                                    EditorGUI.BeginDisabledGroup(true);
                                    EditorGUILayout.Slider($"Layer{i} Time", info.time, 0f, info.length);
                                    EditorGUI.EndDisabledGroup();
                                    EditorGUI.indentLevel--;
                                }
                            }
                ...
                errorLogScrollPosition = EditorGUILayout.BeginScrollView(errorLogScrollPosition);
                {
                    ...
                    #region Warning
                    if (readyWarningsUpdate && e.type == EventType.Layout)
                    {
                        UpdateReadyWarnings();
                        readyWarningsUpdate = false;
                    }
                    if (readyTDOFWarning)
                    {
                        #region TDOF
                        EditorGUILayout.HelpBox(Language.GetText(Language.Help.TranslationDoFisEnableWarning), MessageType.Warning);
                        foreach (var (parent, hi) in readyTDOFWarningBones)
                        {
                            EditorGUILayout.HelpBox(string.Format(Language.GetText(Language.Help.TranslationDoFisdisabled), parent, hi), MessageType.Warning);
                        }
                        #endregion
                    }
                    foreach (var (meshName, topology) in readyMeshWarnings)
                    {
                        EditorGUILayout.HelpBox(string.Format(Language.GetText(Language.Help.MeshTopologyIsNotTriangles), meshName, topology), MessageType.Warning);
                    }
                    #endregion
                }
            ...
            else if (!VA.IsEditError)
            {
                ...
                #region Toolbar
                {
                    EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
                    {
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            EditorPrefs.SetBool(PrefKey_Animation, guiAnimationFoldout);
                        }
                    }
                    {
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            EditorPrefs.SetBool(PrefKey_Tools, guiToolsFoldout);
                        }
                    }
                    {
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            EditorPrefs.SetBool(PrefKey_Settings, guiSettingsFoldout);
                        }
                    }
                    {
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            EditorPrefs.SetBool(PrefKey_Help, guiHelpFoldout);
                        }
                    }
                    {
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            EditorPrefs.SetBool(PrefKey_Preview, guiPreviewFoldout);
                        }
                    }
                    EditorGUILayout.Space();
                    #region Edit
                ...
                if (guiAnimationFoldout)
                {
                    ...
                    }
                    {
                        ...
                        EditorGUILayout.Space();
                        if (GUILayout.Button(UEditorGUI.GUIContents.GetHelpIcon(), guiAnimationHelp ? GuiStyleIconActiveButton : GuiStyleIconButton, GUILayout.Width(19)))
                        {
                ...
                if (guiToolsFoldout)
                {
                    DrawSectionHeader("Tools", ref guiToolsFoldout, ref guiToolsHelp, PrefKey_Tools);
                    if (guiToolsHelp)
                ...
                {
                    DrawSectionHeader("Settings", ref guiSettingsFoldout, ref guiSettingsHelp, PrefKey_Settings);
                    if (guiSettingsHelp)
                ...
                if (guiHelpFoldout)
                {
                    DrawSectionHeader("Help", ref guiHelpFoldout, ref guiHelpHelp, PrefKey_Help);
                    if (guiHelpHelp)
                ...
                {
                    DrawSectionHeader("Preview", ref guiPreviewFoldout, ref guiPreviewHelp, PrefKey_Preview);
                    if (guiPreviewHelp)
        ...
        private void DrawSectionHeader(string label, ref bool foldout, ref bool help, string prefKey)
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginChangeCheck();
            foldout = EditorGUILayout.Foldout(foldout, label, true, GuiStyleBoldFoldout);
            if (EditorGUI.EndChangeCheck())
                EditorPrefs.SetBool(prefKey, foldout);
            EditorGUILayout.Space();
            if (GUILayout.Button(UEditorGUI.GUIContents.GetHelpIcon(), help ? GuiStyleIconActiveButton : GuiStyleIconButton, GUILayout.Width(19)))
                help = !help;
            EditorGUILayout.EndHorizontal();
        }
        ...
        private void VersionInfoGUI()
        {
...

            {
...

                {
                    ...
                    AddPackageText("FBX Exporter");
#endif
                    EditorGUILayout.LabelField(packageText, GuiStyleMiddleRightGreyMiniLabel);
                }
                GUILayout.FlexibleSpace();

                if (GUILayout.Button(GuiContentTutorialVideoPlaylist, EditorStyles.linkLabel))
                {
        ...
        private void UpdateReadyWarnings()
        {
            readyTDOFWarning = false;
            readyTDOFWarningBones.Clear();
            readyMeshWarnings.Clear();

            if (GameObject == null)
                return;

            if (GameObject.activeInHierarchy && Animator != null && Animator.isHuman && Animator.hasTransformHierarchy && VA.UAvatar.GetHasTDoF(Animator.avatar))
            {
                readyTDOFWarning = true;
                ...
                for (int i = 0; i < VeryAnimation.HumanBonesAnimatorTDOFIndex.Length; i++)
                {
                    ...
                    if (Animator.GetBoneTransform(hi) != null)
                    {
                        if (Animator.GetBoneTransform(VeryAnimation.HumanBonesAnimatorTDOFIndex[i].parent) == null)
                            readyTDOFWarningBones.Add((VeryAnimation.HumanBonesAnimatorTDOFIndex[i].parent, hi));
                    }
                    else
                    {
                        readyTDOFWarningBones.Add((hi, hi));
                    }
                }
            }
            ...
            foreach (var smRenderer in GameObject.GetComponentsInChildren<SkinnedMeshRenderer>(true))
            {
                ...
                for (int subMesh = 0; subMesh < smRenderer.sharedMesh.subMeshCount; subMesh++)
                {
                    ...
                    if (topology == MeshTopology.Triangles)
                        continue;
                    readyMeshWarnings.Add((smRenderer.sharedMesh.name, topology));
                }
            }
            foreach (var mfilter in GameObject.GetComponentsInChildren<MeshFilter>(true))
            {
                ...
                for (int subMesh = 0; subMesh < mfilter.sharedMesh.subMeshCount; subMesh++)
                {
                    ...
                    if (topology == MeshTopology.Triangles)
                        continue;
                    readyMeshWarnings.Add((mfilter.sharedMesh.name, topology));
                }
            }
        }
        ...
        {
            if (VA.IsEditError || !IsGuiStyleReady) return;
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
...
#if Enable_Profiler
            Profiler.BeginSample($"****VeryAnimationWindow.OnSceneGUI {Event.current}");
#endif
            ...
            switch (e.type)
            {
                                    ...
                                    if (go != null && VA.BonesIndexOf(go) < 0)
                                    {
                                        ...
                                        bool enable = false;
                                        {
                                            ...
                                            while (t != null)
                                            {
                                                ...
                                                {
                                                    if (t.TryGetComponent<Animator>(out _) || t.TryGetComponent<Animation>(out _))
                                                    {
                ...
                case EventType.MouseDrag:
                    ...
                    {
                        if (GUIUtility.hotControl == 0 && showGizmo)
                        {
                            selectionRect.SetEnd(e.mousePosition);
                            #region Selection
                            {
                                var rect = selectionRect.Rect;
                                bool additiveSelection = Shortcuts.IsKeyControl(e) || e.shift;
                                bool lockBoneSelection = additiveSelection &&
                                    ((selectionRect.beforeSelection != null && selectionRect.beforeSelection.Length > 0) ||
                                     (selectionRect.virtualBeforeSelection != null && selectionRect.virtualBeforeSelection.Length > 0));
                                bool lockAnimatorIKSelection = additiveSelection && !lockBoneSelection &&
                                    selectionRect.beforeAnimatorIKSelection != null && selectionRect.beforeAnimatorIKSelection.Length > 0;
                                bool lockOriginalIKSelection = additiveSelection && !lockBoneSelection && !lockAnimatorIKSelection &&
                                    selectionRect.beforeOriginalIKSelection != null && selectionRect.beforeOriginalIKSelection.Length > 0;
                                #region Now
                                #region Bone
                                {
                                    selectionRect.calcList.Clear();
                                    if (!lockAnimatorIKSelection && !lockOriginalIKSelection)
                                    {
                                ...
                                #region VirtualBone
                                {
                                    ...
                                    if (VA.IsHuman && !lockAnimatorIKSelection && !lockOriginalIKSelection)
                                    {
                                ...
                                #region AnimatorIK
                                {
                                    ...
                                    if (VA.IsHuman && !lockBoneSelection && !lockOriginalIKSelection && selectionRect.calcList.Count == 0 && selectionRect.virtualCalcList.Count == 0)
                                    {
                                ...
                                #region OriginalIK
                                {
                                    ...
                                    if (!lockBoneSelection && !lockAnimatorIKSelection && selectionRect.calcList.Count == 0 && selectionRect.virtualCalcList.Count == 0 && selectionRect.animatorIKCalcList.Count == 0)
                                    {
                    ...
                    else if (GUIUtility.hotControl == 0 && selectionRect.Enable && selectionRect.Distance < 10f)
                    {
                        #region SelectMesh
                        {
                            ...
                            int originalIKTarget = -1;
                            {
                                ...
                                void SetPickCandidate(Transform bone, float lengthSq)
                                {
                                ...
                                foreach (var renderer in VA.Renderers)
                                {
                                    ...
                                    if (renderer is SkinnedMeshRenderer)
                                    {
                                        ...
                                        if (skinnedMeshRenderer.sharedMesh != null)
                                        {
                                            ...
                                            var worldToLocalMatrix = Matrix4x4.TRS(rt.position, rt.rotation, Vector3.one).inverse;
                                                ...
                                                for (int i = 0; i < triangles.Count; i += 3)
                                                {
                                                    ...
                                                    Transform bone = null;
                                                    {
                                                        ...
                                                        void AddBonePoint(int boneIndex, float boneWeight)
                                                        {
                                                            ...
                                                            if (bonePoints.TryGetValue(boneIndex, out float existingPoint))
                                                                bonePoints[boneIndex] = Mathf.Min(existingPoint, point);
                                                            else
                                                    ...
                                                    if (bone != null)
                                                    {
                                                        SetPickCandidate(bone, lengthSq);
                                                    }
                                                }
                                            }
                                        ...
                                        if (meshFilter != null && meshFilter.sharedMesh != null)
                                        {
                                            ...
                                            for (int subMesh = 0; subMesh < mesh.subMeshCount; subMesh++)
                                            {
                                                ...
                                                for (int i = 0; i < triangles.Count; i += 3)
                                                {
                                                    ...
                                                    SetPickCandidate(renderer.transform, lengthSq);
                                                }
                                            }
                                        }
                                        #endregion
            ...
            if (showGizmo)
            {
                #region Handle
                {
                            ...
                            else if (currentTool == Tool.Rotate)
                            {
                                ...
                                if (EditorGUI.EndChangeCheck())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        bodyRotation = Quaternion.AngleAxis(angle, handleRotation * axis) * bodyRotation;
                            ...
                            if (currentTool == Tool.Move)
                            {
                                ...
                                if (EditorGUI.EndChangeCheck())
                                {
                                    void ChangeTDOF(HumanBodyBones hi, Vector3 move)
                                    {
                                        ...
                                        {
                                            var mat = (VA.Skeleton.GameObject.transform.worldToLocalMatrix * VA.Skeleton.HumanoidBones[(int)hi].transform.localToWorldMatrix).inverse;
                                            EditorCommon.GetTRS(mat, out _, out _, out Vector3 lscale);
                    ...
                    if (genericHandle && VA.SelectionActiveBone >= 0)
                    {
                        ...
                        if (handleTransformUpdate)
                        {
                            ...
                            {
                                var activeBoneTr = VA.Skeleton.Bones[VA.SelectionActiveBone].transform;
                                handlePosition = activeBoneTr.position;
                                if (Tools.pivotRotation == PivotRotation.Local)
                                    handleRotation = activeBoneTr.rotation;
                                else
                                    handleRotation = Tools.handleRotation;
                            }
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                if (Tools.pivotMode == PivotMode.Center)
                                {
                                    ...
                                    SetCenterRotationAction((boneIndex, different) =>
                                    {
                                        ...
                                        {
                                            VA.UAw.GetTimelineRootMotionOffsets(out _, out Quaternion offsetRotation);
                        ...
                        muscleRotationSliderIds != null && muscleRotationSliderIds.Length == 3)
                    {
                        ...
                        if (humanoidIndex >= 0 && VA.CurrentTool() == Tool.Rotate)
                        {
                            ...
                            if (index1 != -1)
                            {
                                ...
                                if (VA.MusclePropertyName.Names[index1].StartsWith("Left", StringComparison.Ordinal) || VA.MusclePropertyName.Names[index1].StartsWith("Right", StringComparison.Ordinal)) //why?
                                {
                                    angle *= 0.5f;
                                    num *= 0.5f;
                                }
                            ...
                            void DrawArcX()
                            {
                                if (index1 == -1)
                                    return;
                                Color color = muscleRotationHandleIds[0] == GUIUtility.hotControl || muscleRotationSliderIds[0] == GUIUtility.hotControl ? HandleColorActive : HandleColorInactive;
                            ...
                            void DrawArcY()
                            {
                                if (index2 == -1)
                                    return;
                                Color color = muscleRotationHandleIds[1] == GUIUtility.hotControl || muscleRotationSliderIds[1] == GUIUtility.hotControl ? HandleColorActive : HandleColorInactive;
                            ...
                            void DrawArcZ()
                            {
                                if (index3 == -1)
                                    return;
                                Color color = muscleRotationHandleIds[2] == GUIUtility.hotControl || muscleRotationSliderIds[2] == GUIUtility.hotControl ? HandleColorActive : HandleColorInactive;
                ...
                #region Bones
                {
                    ...
                    for (int i = 0; i < VA.Bones.Length; i++)
                    {
                        ...
                        if (VA.IsHuman)
                        {
                            if (VA.BoneIndex2humanoidIndex[i] < 0)
                                guiStyle = GuiStyleDiamondButton;
                        }
                        else
                        {
                            if (VA.RootMotionBoneIndex < 0 && i == 0)
...
#endif
            #region IK
            if (VA.SkeletonIKShowBoneList != null && VA.SkeletonIKShowBoneList.Count > 0)
            {
                if (EditorSettings.SettingsSkeletonIKType == EditorSettings.SkeletonType.Line)
                {
                    UHandleUtility.ApplyWireMaterial();
        ...
        {
            if (boneIndex < 0 || !VA.SkeletonFKShowBoneFlag[boneIndex])
                return;

            var parentIndex = targetIndex >= 0 ? targetIndex : VA.ParentBoneIndexes[boneIndex];
            if (parentIndex < 0)
                return;

            var boneA = VA.Bones[parentIndex];
        ...
        private void OnInspectorUpdate()
        {
            ...
            else
            {
                ...
                #region Repaint
                {
                    var errorCode = VA.CurrentErrorCode;
        ...
        private void CustomUpdate()
        {
            if (Initialized)
            {
                if (VA.IsEditError)
                {
                    ...
                    {
                        Debug.LogFormat("<color=blue>[Very Animation]</color>Editing ended : {0}", errorCode);
                    }
                }
        ...
        public void Initialize()
        {
            ...
            if (VeryAnimationEditorWindow.instance == null)
            {
                ...
                {
                    var window = EditorCommon.FindResourceFirst<EditorWindow>(w => w.GetType().Name == "InspectorWindow" && w.hasFocus);
                    if (window != null)
            ...
            {
                var dockWindow = EditorCommon.FindResourceFirst<EditorWindow>(w =>
                {
            ...
            #region SaveSettings
            {
                #region EditorPref
                {
                    guiAnimationFoldout = EditorPrefs.GetBool(PrefKey_Animation, true);
                    guiToolsFoldout = EditorPrefs.GetBool(PrefKey_Tools, false);
                    guiSettingsFoldout = EditorPrefs.GetBool(PrefKey_Settings, false);
                    guiHelpFoldout = EditorPrefs.GetBool(PrefKey_Help, false);
                    guiPreviewFoldout = EditorPrefs.GetBool(PrefKey_Preview, true);
                }
                #endregion
                VA.LoadSaveSettings();
            }
        ...
        {
            var index = ClipSelectorTreeView.AnimationClips.IndexOf(clip);
            return index >= 0;
        }
        ...
        private void SetGameObject()
        {
            ...
            if (ap is Animator apAnimator)
            {
                if (Animator != apAnimator)
                {
                    Animator = apAnimator;
                    updated = true;
                }
            ...
            else if (ap is Animation apAnimation)
            {
                if (Animation != apAnimation)
                {
                    Animation = apAnimation;
                    updated = true;
                }
                Animator = null;
            }
