...
{
    internal sealed class SynchronizeAnimation : IDisposable
    {
        ...
        public SynchronizeAnimation()
        {
            ...
            void AddGameObject(GameObject go)
            {
                ...
                {
                    if (animator == null || animator == VAW.Animator || animator.gameObject == VAW.GameObject)
                        continue;
                    ...
                    AnimationClip clip = null;
                    {
                        ...
                        if (clip == null)
                        {
                            var ac = AnimationCommon.GetAnimatorController(animator);
                ...
                {
                    if (animation == null || animation == VAW.Animation || animation.gameObject == VAW.GameObject)
                        continue;
        ...
        public void UpdateSameClip(AnimationClip clip)
        {
            foreach (var data in synchronizeObjects)
            {
                if (data == null || data.clip != clip)
                    continue;
                data.animationClipValueSave.AddBindings(clip);
                SampleAnimation(data);
            }
        }
    }
