...
{
    internal sealed class DummyObject : IDisposable
    {
        ...
        private List<Material> createdDepthMaterials;
        private List<TransparentDepthRenderer> transparentDepthRenderers;
        ...
        private struct TransparentDepthRenderer
        {
            public Renderer colorRenderer;
            public Renderer depthRenderer;
            public SkinnedMeshRenderer colorSkinned;
            public SkinnedMeshRenderer depthSkinned;
        }
        ...
        private static readonly int ShaderID_ColorMask = Shader.PropertyToID("_ColorMask");
        private static readonly int ShaderID_ZWrite = Shader.PropertyToID("_ZWrite");
        private static readonly int ShaderID_SrcBlend = Shader.PropertyToID("_SrcBlend");
        private static readonly int ShaderID_DstBlend = Shader.PropertyToID("_DstBlend");
        private static int DepthPrimeRenderQueue
        {
            get
            {
                if (EditorCommon.IsHighDefinitionRenderPipeline())
                    return (int)UnityEngine.Rendering.RenderQueue.GeometryLast;
                return (int)UnityEngine.Rendering.RenderQueue.Transparent - 400;
            }
        }
        ...
        public void Initialize(GameObject sourceObject)
        {
            ...
            GameObject = AnimationCommon.InstantiateForPreview(sourceObject);
            ...
            if (Animator != null)
            {
                if (sourceObject.TryGetComponent<Animator>(out _))
                {
            ...
            if (Animation != null)
            {
                if (sourceObject.TryGetComponent<Animation>(out _))
                {
            ...
            #region rendererDictionary
            {
                ...
                var objectRendererPathMap = new Dictionary<string, Renderer>(objectRenderers.Length);
                foreach (var renderer in objectRenderers)
                {
                    if (renderer == null)
                        continue;
                    var path = AnimationUtility.CalculateTransformPath(renderer.transform, GameObject.transform);
                    objectRendererPathMap.TryAdd(path, renderer);
                }
                foreach (var sr in sourceRenderers)
                {
                    ...
                    var found = objectRendererPathMap.TryGetValue(spath, out var objectRenderer);
                    Assert.IsTrue(found);
                    if (found)
                        rendererDictionary.TryAdd(objectRenderer, sr);
                }
            }
            #endregion
        ...
        public void ResetTransformRoot()
        {
        ...
        public void ChangeTransparent()
        {
            ...
            createdDepthMaterials = new List<Material>();
            transparentDepthRenderers = new List<TransparentDepthRenderer>();
            foreach (var pair in rendererDictionary)
            {
                ...
                if (!changeShader && pair.Key is MeshRenderer)
                {
                    ...
                    {
                        if (comp.GetType().Name.StartsWith("TextMesh", StringComparison.Ordinal))
                        {
                            changeShader = false;
                            break;
                        }
                ...
                if (changeShader)
                {
                    var shader = Shader.Find("Hidden/Very Animation/OnionSkin");
                    ...
                    CreateTransparentDepthRenderer(pair.Key, materials);
                }
                else
                {
                    var materials = new Material[pair.Key.sharedMaterials.Length];
        ...
        private void CreateTransparentDepthRenderer(Renderer colorRenderer, Material[] colorMaterials)
        {
            var depthMaterials = new Material[colorMaterials.Length];
            for (int i = 0; i < depthMaterials.Length; i++)
            {
                var colorMaterial = colorMaterials[i];
                if (colorMaterial == null)
                    continue;
                var depthMaterial = new Material(colorMaterial);
                depthMaterial.hideFlags |= HideFlags.HideAndDontSave;
                depthMaterial.SetFloat(ShaderID_ColorMask, 0f);
                depthMaterial.SetFloat(ShaderID_ZWrite, 1f);
                depthMaterial.SetFloat(ShaderID_SrcBlend, (float)UnityEngine.Rendering.BlendMode.One);
                depthMaterial.SetFloat(ShaderID_DstBlend, (float)UnityEngine.Rendering.BlendMode.Zero);
                createdDepthMaterials.Add(depthMaterial);
                depthMaterials[i] = depthMaterial;
            }

            var depthObject = new GameObject($"{colorRenderer.name}_TransparentDepth");
            depthObject.hideFlags |= HideFlags.HideAndDontSave;
            depthObject.transform.SetParent(colorRenderer.transform, false);

            Renderer depthRenderer;
            var colorSkinned = colorRenderer as SkinnedMeshRenderer;
            SkinnedMeshRenderer depthSkinned = null;
            if (colorSkinned != null)
            {
                depthSkinned = depthObject.AddComponent<SkinnedMeshRenderer>();
                depthSkinned.sharedMesh = colorSkinned.sharedMesh;
                depthSkinned.bones = colorSkinned.bones;
                depthSkinned.rootBone = colorSkinned.rootBone;
                depthSkinned.localBounds = colorSkinned.localBounds;
                depthSkinned.quality = colorSkinned.quality;
                depthSkinned.updateWhenOffscreen = colorSkinned.updateWhenOffscreen;
                depthRenderer = depthSkinned;
            }
            else
            {
                var meshFilter = depthObject.AddComponent<MeshFilter>();
                if (colorRenderer.TryGetComponent<MeshFilter>(out var sourceMeshFilter))
                    meshFilter.sharedMesh = sourceMeshFilter.sharedMesh;
                depthRenderer = depthObject.AddComponent<MeshRenderer>();
            }
            depthRenderer.sharedMaterials = depthMaterials;
            depthRenderer.enabled = colorRenderer.enabled;

            transparentDepthRenderers.Add(new TransparentDepthRenderer()
            {
                colorRenderer = colorRenderer,
                depthRenderer = depthRenderer,
                colorSkinned = colorSkinned,
                depthSkinned = depthSkinned,
            });
        }
        public void UpdateTransparentDepth()
        {
            if (transparentDepthRenderers == null)
                return;
            foreach (var tdr in transparentDepthRenderers)
            {
                if (tdr.colorRenderer == null || tdr.depthRenderer == null)
                    continue;
                if (tdr.depthRenderer.enabled != tdr.colorRenderer.enabled)
                    tdr.depthRenderer.enabled = tdr.colorRenderer.enabled;
                if (tdr.colorSkinned != null && tdr.depthSkinned != null && tdr.colorSkinned.sharedMesh != null)
                {
                    var count = tdr.colorSkinned.sharedMesh.blendShapeCount;
                    for (int i = 0; i < count; i++)
                    {
                        var weight = tdr.colorSkinned.GetBlendShapeWeight(i);
                        if (tdr.depthSkinned.GetBlendShapeWeight(i) != weight)
                            tdr.depthSkinned.SetBlendShapeWeight(i, weight);
                    }
                }
            }
        }
        public void RevertTransparent()
        {
            if (createdMaterials != null)
            {
                if (transparentDepthRenderers != null)
                {
                    foreach (var tdr in transparentDepthRenderers)
                    {
                        if (tdr.depthRenderer != null)
                            GameObject.DestroyImmediate(tdr.depthRenderer.gameObject);
                    }
                    transparentDepthRenderers = null;
                }
                if (createdDepthMaterials != null)
                {
                    foreach (var mat in createdDepthMaterials)
                    {
                        if (mat != null)
                            Material.DestroyImmediate(mat);
                    }
                    createdDepthMaterials = null;
                }
        ...
        public void SetTransparentRenderQueue(int renderQueue)
        {
            ...
            if (createdDepthMaterials != null)
            {
                foreach (var mat in createdDepthMaterials)
                {
                    mat.renderQueue = DepthPrimeRenderQueue;
                }
            }
        }
        ...
        public void RendererDestroyImmediate()
        {
            ...
            resetOriginalBlendShapeCaches = Array.Empty<ResetOriginalBlendShapeCache>();
        }
        ...
        public void RendererForceUpdate()
        {
            ...
            if (transparentDepthRenderers != null)
            {
                foreach (var tdr in transparentDepthRenderers)
                {
                    if (tdr.depthRenderer == null) continue;
                    tdr.depthRenderer.enabled = !tdr.depthRenderer.enabled;
                    tdr.depthRenderer.enabled = !tdr.depthRenderer.enabled;
                }
            }
        }
        ...
        private void UpdateBones()
        {
            #region Humanoid
            HumanPoseHandler?.Dispose();
            if (VAW.VA.IsHuman && Animator != null && Animator.avatar != null)
            {
                if (!Animator.isInitialized)
                    Animator.Rebind();
                ...
                HumanPoseHandler = AnimationCommon.CreateHumanPoseHandler(Animator);
            }
            else
            {
                HumanoidBones = null;
