...
namespace VeryAnimation
{
    ...
    internal class AnimatorIKCore
    {
        ...
        private GameObject HumanoidBone(HumanBodyBones hi) => VAW.VA.Skeleton.HumanoidBones[(int)hi];

        #region Undo Strings
        private const string UndoChangeAnimatorIKData = "Change Animator IK Data";
        ...
        private const string UndoChangeIKSettings = "Change IK Settings";
        ...
        private const string UndoChangeAnimationRiggingConstraint = "Change Animation Rigging Constraint";
        ...
        class GUIStyles
        {
            public readonly GUIContent guiContentWeightButton;
            public readonly GUIContent guiContentAdvancedFoldout;
            public readonly GUIContent[] ikSpaceTypeStrings = new GUIContent[(int)AnimatorIKData.SpaceType.Total];

            public GUIStyles()
            {
                guiContentWeightButton = new GUIContent("Weight", "IRigConstraint.weight");
                guiContentAdvancedFoldout = new GUIContent(" ", "Advanced");
                UpdateLanguage();
        ...
        private static readonly HumanBodyBones[] IKTargetToTipBone =
        {
            HumanBodyBones.Head,
...
#if VERYANIMATION_ANIMATIONRIGGING
        private static readonly HumanBodyBones[,] IKTargetToTwoBoneIK =
        {
            { HumanBodyBones.LastBone,      HumanBodyBones.LastBone,      HumanBodyBones.LastBone  },
...
#endif
        private static readonly Quaternion IKSyncRotationHead = Quaternion.identity;
        private static readonly Quaternion IKSyncRotationLeftHand = Quaternion.Euler(0, 90, 180);
        private static readonly Quaternion IKSyncRotationRightHand = Quaternion.Euler(0, 90, 0);
        private static readonly Quaternion IKSyncRotationFoot = Quaternion.Euler(90, 0, 90);

        private static readonly Quaternion[] IKTargetSyncRotation =
        {
            IKSyncRotationHead,
            IKSyncRotationLeftHand,
            IKSyncRotationRightHand,
            IKSyncRotationFoot,
            IKSyncRotationFoot,
        };

        private Vector3 GetFeetBottomOffset(IKTarget target, float sign)
        {
            return target switch
            {
                IKTarget.LeftFoot => new Vector3(0f, sign * VAW.VA.Skeleton.Animator.leftFeetBottomHeight, 0f),
                IKTarget.RightFoot => new Vector3(0f, sign * VAW.VA.Skeleton.Animator.rightFeetBottomHeight, 0f),
                _ => Vector3.zero,
            };
        }
        ...
        internal class AnimatorIKData
        {
                ...
                switch (spaceType)
                {
                    ...
                    case SpaceType.Local:
                        if (parentBoneIndex >= 0 && VAW.VA.Bones[parentBoneIndex] != null)
                            return VAW.VA.Bones[parentBoneIndex].transform;
                        return null;
            ...
            public bool IsUpdate => enable && updateIKtarget && !synchroIKtarget;
        ...
        public IKTarget IKActiveTarget => ikTargetSelect != null && ikTargetSelect.Length > 0 ? ikTargetSelect[0] : IKTarget.None;
        ...
        public void Initialize()
        {
            ...
            if (VAW.VA.IsHuman)
            {
                ...
                for (IKTarget target = IKTarget.LeftHand; target <= IKTarget.RightHand; target++)
                {
                    ...
                    if (HumanoidBone(shoulderIndex) == null)
                        continue;

                    defaultPoseShoulderToHandLength[index] = (HumanoidBone(handIndex).transform.position - HumanoidBone(shoulderIndex).transform.position).magnitude;
                    ...
                    armStretchToMaximumRate[index] = defaultPoseShoulderToHandLength[index] > 0f ? axisLength / defaultPoseShoulderToHandLength[index] : 1f;
                }
        ...
        private void UpdateReorderableList()
        {
            ...
            ikReorderableList = new ReorderableList(ikData, typeof(AnimatorIKData), draggable: false, displayHeader: false, displayAddButton: false, displayRemoveButton: false);
            ikReorderableList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                ...
                if (ikData[index].rigConstraint != null)
                {
                    ...
                    if (GUI.Button(r, ikData[index].rigConstraintGUIContent))
                    {
                        VAW.VA.SelectGameObject(ikData[index].rigConstraint.gameObject);
                        {
                            ...
                            VAW.VA.SetAnimationWindowSynchroSelection(list);
                        }
                    }
                }
#endif

                {
                    ...
                    EditorGUI.LabelField(r, Styles.ikSpaceTypeStrings[(int)ikData[index].spaceType], VAW.GuiStyleMiddleRightGreyMiniLabel);
                }
...
#endif
                {
                    ...
                    r.width = 24;
                    r.x = rect.xMax - 12;
                    advancedFoldout = EditorGUI.Foldout(r, advancedFoldout, Styles.guiContentAdvancedFoldout, true);
                }
            };
        ...
        public void SynchroSet(IKTarget target, SynchroSetFlags syncFlags = SynchroSetFlags.Default)
        {
                ...
                case AnimatorIKData.SpaceType.Local:
                    switch (target)
                    {
                        case IKTarget.Head:
                            ...
                            if (data.parentBoneIndex < 0)
                                data.parentBoneIndex = VAW.VA.HumanoidIndex2boneIndex[(int)HumanBodyBones.Spine];
                            break;
            ...
            void GetSwivelRotation(Vector3 posStart, Vector3 posEnd, Vector3 posCenter, out float swivelRotation)
            {
                ...
                if (axis.sqrMagnitude > 0f)
                {
...

                    {
                        ...
                        swivelRotation = EditorCommon.WrapAngle(rot.eulerAngles.z);
                    }
                    var basicSwivelRotation = 0f;
                    {
                        ...
                        var rot = Quaternion.FromToRotation(Vector3.up, vec);
                        basicSwivelRotation = EditorCommon.WrapAngle(rot.eulerAngles.z);
                    }
                    if (Mathf.Abs(swivelRotation - basicSwivelRotation) > EditorCommon.HalfRotation)
                    {
                        if (swivelRotation < basicSwivelRotation)
                            swivelRotation += EditorCommon.FullRotation;
                        else
                            basicSwivelRotation += EditorCommon.FullRotation;
                    }
                    ...
                    swivelRotation = EditorCommon.WrapAngleSigned(swivelRotation);
                }
            }
            ...
            switch (target)
            {
                ...
                case IKTarget.RightFoot:
                    ...
                    if (!done && syncFlags.HasFlag(SynchroSetFlags.AnimationRigging))
                    {
                        ...
                        {
                            var tStart = HumanoidBone(hiStart).transform;
                            var tEnd = HumanoidBone(hiEnd).transform;
                            ...
                            {
                                {
                                    ...
                                    position += GetFeetBottomOffset(target, +1f);
                                    #endregion
                                }
                                {
                    ...
                    if (!done && syncFlags.HasFlag(SynchroSetFlags.HumanoidIK))
                    {
                        var tStart = HumanoidBone(hiStart).transform;
                        var tEnd = HumanoidBone(hiEnd).transform;
                        var tCenter = HumanoidBone(hiCenter).transform;
            ...
            switch (data.spaceType)
            {
                ...
                case AnimatorIKData.SpaceType.Local:
                    ...
                    data.swivelRotation = EditorCommon.WrapAngleSigned(swivelRotation);
                    break;
                case AnimatorIKData.SpaceType.Parent:
                    //not update
        ...
        public string GetSynchroInfoToString(IKTarget target, bool writeOnUpdate = false)
        {
            var data = ikData[(int)target];
            if (!writeOnUpdate)
                return $"Sync({data.defaultSyncType})";
            return $"Sync({data.defaultSyncType}), Write on Update({GetWriteOnUpdateLabel(target, data.writeFlags)})";

            static string GetWriteOnUpdateLabel(IKTarget target, AnimatorIKData.WriteFlags writeFlags)
            {
                switch (target)
                {
                    case IKTarget.Head:
                        return writeFlags.HasFlag(AnimatorIKData.WriteFlags.AnimationRiggingTarget) ? "Target" : "[Nothing]";
                    case IKTarget.LeftHand:
                    case IKTarget.RightHand:
                    case IKTarget.LeftFoot:
                    case IKTarget.RightFoot:
                        var hasTarget = writeFlags.HasFlag(AnimatorIKData.WriteFlags.AnimationRiggingTarget);
                        var hasHint = writeFlags.HasFlag(AnimatorIKData.WriteFlags.AnimationRiggingHint);
                        if (hasTarget && hasHint) return "Target, Hint";
        ...
        public bool IsValid(IKTarget target)
        {
            ...
            while (boneIndex >= 0)
            {
                var defaultPose = VAW.VA.BoneDefaultPose[boneIndex];
                if (defaultPose == null)
                    return false;
                var defaultScale = defaultPose.scale;
                var currentScale = VAW.VA.Bones[boneIndex].transform.localScale;
                if (Mathf.Abs(defaultScale.x - currentScale.x) > 0.0001f ||
                    Mathf.Abs(defaultScale.y - currentScale.y) > 0.0001f ||
                    Mathf.Abs(defaultScale.z - currentScale.z) > 0.0001f)
                    return false;
                boneIndex = VAW.VA.ParentBoneIndexes[boneIndex];
            }
        ...
        public void UpdateIK(bool rootUpdated, int defaultLoopCount = 1)
        {
            if (!VAW.VA.IsHuman) return;
            ...
            #region BeforeIK
            {
                ...
                if (ikData[(int)IKTarget.Head].IsUpdate)
                {
                    ...
                    AnimationCommon.SetKeyframe(muscleCurves[neckMuscleIndexes[0]], VAW.VA.CurrentTime, 0f);
                    AnimationCommon.SetKeyframe(muscleCurves[headMuscleIndexes[0]], VAW.VA.CurrentTime, 0f);
                    {
                        float angle = (VAW.VA.HumanoidMuscleLimit[(int)HumanBodyBones.Neck].max.y - VAW.VA.HumanoidMuscleLimit[(int)HumanBodyBones.Neck].min.y) / 2f;
                        var rate = (-ikData[(int)IKTarget.Head].swivelRotation / angle) / 2f;
                        AnimationCommon.SetKeyframe(muscleCurves[neckMuscleIndexes[1]], VAW.VA.CurrentTime, rate);
                    }
                    {
                        float angle = (VAW.VA.HumanoidMuscleLimit[(int)HumanBodyBones.Head].max.y - VAW.VA.HumanoidMuscleLimit[(int)HumanBodyBones.Head].min.y) / 2f;
                        var rate = (-ikData[(int)IKTarget.Head].swivelRotation / angle) / 2f;
                        AnimationCommon.SetKeyframe(muscleCurves[headMuscleIndexes[1]], VAW.VA.CurrentTime, rate);
                    }
                    {
                        var rate = muscleCurves[neckMuscleIndexes[2]].Evaluate(VAW.VA.CurrentTime);
                        rate = Mathf.Clamp(rate, -1f, 1f);
                        AnimationCommon.SetKeyframe(muscleCurves[neckMuscleIndexes[2]], VAW.VA.CurrentTime, rate);
                    }
                    {
                        var rate = muscleCurves[headMuscleIndexes[2]].Evaluate(VAW.VA.CurrentTime);
                        rate = Mathf.Clamp(rate, -1f, 1f);
                        AnimationCommon.SetKeyframe(muscleCurves[headMuscleIndexes[2]], VAW.VA.CurrentTime, rate);
                    }
                ...
                for (IKTarget target = IKTarget.LeftHand; target <= IKTarget.RightHand; target++)
                {
                    ...
                    var posShoulder = HumanoidBone(shoulderIndex).transform.position;
                    ...
                    //muscleY
                    {
                        ...
                        //Bend
                        if (defaultPoseShoulderToHandLength[index] > 0f)
                        {
                        ...
                        AnimationCommon.SetKeyframe(muscleCurves[muscleIndex], VAW.VA.CurrentTime, muscleValue);
                        VAW.VA.SetAnimationCurveAnimatorMuscle(muscleIndex, muscleCurves[muscleIndex]);
                    }
            ...
            for (int loop = 0; loop < loopCount; loop++)
            {
                ...
                #region Options
                {
                    ...
                    #region Virtual Neck
                    if (ikData[(int)IKTarget.Head].IsUpdate && HumanoidBone(HumanBodyBones.Neck) == null)
                    {
                    ...
                    VAW.VA.Skeleton.ResetTransformRoot();
                    VAW.VA.Skeleton.HumanPoseHandler.SetHumanPose(ref hp);
                }
                #endregion
                ...
                #region SetKeyframe
                {
                    ...
                    for (int i = 0; i < hp.muscles.Length; i++)
                    {
                        ...
                        AnimationCommon.SetKeyframe(muscleCurves[i], VAW.VA.CurrentTime, hp.muscles[i]);
                    ...
                    if (rootWrite)
                    {
                        for (int i = 0; i < 3; i++)
                        {
                            ...
                            AnimationCommon.SetKeyframe(rootTCurves[i], VAW.VA.CurrentTime, hp.bodyPosition[i]);
                        }
                        for (int i = 0; i < 4; i++)
                        {
                            if (rootQCurves[i] == null)
                                rootQCurves[i] = VAW.VA.GetAnimationCurveAnimatorRootQ(i);
                            AnimationCommon.SetKeyframe(rootQCurves[i], VAW.VA.CurrentTime, hp.bodyRotation[i]);
                        }
                    }
                }
                #endregion
        ...
        public void HandleGUI()
        {
            ...
            if (IKActiveTarget != IKTarget.None && ikData[(int)IKActiveTarget].enable)
            {
                ...
                {
                    {
                        var posA = HumanoidBone(hiA).transform.position;
                        ...
                        if (axis.sqrMagnitude > 0f)
                        {
                            ...
                            if (IKActiveTarget == IKTarget.Head)
                            {
                                ...
                                var tpos = posP + right;
                                posPC = tpos - axis * Vector3.Dot(tpos - posP, axis);
                                lineDir = (posPC - posP).normalized;
                                discColor = Handles.yAxisColor;
                            }
                            else
                            {
                                posPC = posA + axis * Vector3.Dot(activeData.swivelPosition - posA, axis);
                                lineDir = (activeData.swivelPosition - posPC).normalized;
                                discColor = Handles.zAxisColor;
                            }
                            ...
                            Handles.DrawLine(posP, posP + lineDir * handleSize);
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                {
                                    var data = ikData[(int)target];
                                    data.swivelRotation = EditorCommon.WrapAngleSigned(data.swivelRotation - rotDist);
                                    UpdateSwivelPosition(target);
                    ...
                    if (!activeData.autoRotation &&
                        ((IKActiveTarget == IKTarget.LeftFoot && HumanoidBone(HumanBodyBones.LeftToes) != null) ||
                       (IKActiveTarget == IKTarget.RightFoot && HumanoidBone(HumanBodyBones.RightToes) != null)))
                    {
                        {
                            Handles.color = Handles.centerColor;
                        ...
                        }
                        {
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                var toesTransform = IKActiveTarget == IKTarget.LeftFoot ? HumanoidBone(HumanBodyBones.LeftToes).transform : HumanoidBone(HumanBodyBones.RightToes).transform;
                                ...
                                if (beforeVec.sqrMagnitude > 0f && afterVec.sqrMagnitude > 0f)
                                {
                                    ...
                                    foreach (var target in ikTargetSelect)
                                    {
                                        ...
                                        {
                                            var toesBone = HumanoidBone(target == IKTarget.LeftFoot ? HumanBodyBones.LeftToes : HumanBodyBones.RightToes);
                                            if (toesBone != null)
                                            {
                                                toesBone.transform.rotation = Quaternion.Inverse(moveRotation) * toesBone.transform.rotation;
        ...
        public void TargetGUI()
        {
            ...
            for (int target = 0; target < (int)IKTarget.Total; target++)
            {
                ...
                if (ikTargetSelect != null &&
                    ArrayUtility.Contains(ikTargetSelect, ikTarget))
                {
                    #region Active
                    {
                        if (ikTarget == IKActiveTarget)
                        {
                            ...
                            {
                                if (HumanoidBone(HumanBodyBones.Neck) != null)
                                    hiA2 = HumanBodyBones.Neck;
                                else
                                    hiA2 = HumanBodyBones.Head;
                            }
                            Vector3 worldPosition2 = HumanoidBone(hiA2).transform.position;
        ...
        public void SelectionGUI()
        {
            ...
            #region IK
            {
                ...
                #region Mirror
                {
                    ...
                    if (GUILayout.Button(Language.GetContentFormat(Language.Help.SelectionMirror, (mirrorTarget != IKTarget.None ? $"From 'IK: {IKTargetStrings[(int)mirrorTarget]}'" : "From self"))))
                    {
            ...
            #region SpaceType
            {
                ...
                EditorGUI.BeginChangeCheck();
                var spaceType = (AnimatorIKData.SpaceType)GUILayout.Toolbar((int)activeData.spaceType, Styles.ikSpaceTypeStrings, EditorStyles.miniButton);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObject(VAW, UndoChangeIKSettings);
                    foreach (var target in ikTargetSelect)
                    {
            ...
            #region Swivel
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        var data = ikData[(int)target];
                        data.swivelRotation = EditorCommon.WrapAngleSigned(data.swivelRotation + move);
                        UpdateSwivelPosition(target);
                        VAW.VA.SetUpdateIKtargetAnimatorIK(target);
                    }
                }
            ...
            if (activeData.rigConstraint != null)
            {
                ...
                EditorGUILayout.BeginHorizontal(RowCount++ % 2 == 0 ? VAW.GuiStyleAnimationRowEvenStyle : VAW.GuiStyleAnimationRowOddStyle);
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObject(VAW, UndoChangeAnimatorIKData);
                        foreach (var target in ikTargetSelect)
                        {
                            var data = ikData[(int)target];
                ...
                {
                    EditorGUILayout.BeginHorizontal(RowCount++ % 2 == 0 ? VAW.GuiStyleAnimationRowEvenStyle : VAW.GuiStyleAnimationRowOddStyle);
                    if (GUILayout.Button(Styles.guiContentWeightButton, GUILayout.Width(128)))
                    {
                        var list = new List<EditorCurveBinding>();
                        foreach (var target in ikTargetSelect)
                        {
                ...
                #endregion
                #region Rotation
                if (IKActiveTarget >= IKTarget.LeftHand && IKActiveTarget <= IKTarget.RightFoot && targetBoneIndex >= 0)
                {
                    EditorGUILayout.BeginHorizontal(RowCount++ % 2 == 0 ? VAW.GuiStyleAnimationRowEvenStyle : VAW.GuiStyleAnimationRowOddStyle);
                ...
                #endregion
                #region Position
                if (IKActiveTarget >= IKTarget.LeftHand && IKActiveTarget <= IKTarget.RightFoot && hintBoneIndex >= 0)
                {
                    EditorGUILayout.BeginHorizontal(RowCount++ % 2 == 0 ? VAW.GuiStyleAnimationRowEvenStyle : VAW.GuiStyleAnimationRowOddStyle);
            ...
            if (ikReorderableList != null)
            {
                        ...
                        #region AnimationRigging
                        {
                            ...
                            GUILayout.FlexibleSpace();
                            {
                                ...
                                if (GUILayout.Button("Create"))
                                {
                                    EditorApplication.delayCall += () =>
                                    {
                                        Undo.RecordObject(VAW, UndoChangeAnimationRiggingConstraint);
                ...
                }
                {
                    ...
                    if (GUILayout.Button(Language.GetContent(Language.Help.AnimatorIKSelectAll)))
                    {
                        ...
                        VAW.VA.SelectIKTargets(list, null);
                    }
                    EditorGUILayout.EndHorizontal();
                }
            }
        ...
        private void Reset(IKTarget target)
        {
            ...
            switch (target)
            {
                ...
                case IKTarget.LeftHand:
                    {
                        var posA = HumanoidBone(HumanBodyBones.LeftUpperArm).transform.position;
                        var posB = HumanoidBone(HumanBodyBones.LeftHand).transform.position;
                        var posC = HumanoidBone(HumanBodyBones.LeftLowerArm).transform.position;
                        var forward = posB - posC;
                        var up = data.WorldPosition - Vector3.Lerp(posA, posB, 0.5f);
                        if (forward.sqrMagnitude > 0f && up.sqrMagnitude > 0f)
                            data.WorldRotation = Quaternion.LookRotation(forward, up);
                    }
                    break;
                case IKTarget.RightHand:
                    {
                        var posA = HumanoidBone(HumanBodyBones.RightUpperArm).transform.position;
                        var posB = HumanoidBone(HumanBodyBones.RightHand).transform.position;
                        var posC = HumanoidBone(HumanBodyBones.RightLowerArm).transform.position;
                        var forward = posB - posC;
                        var up = data.WorldPosition - Vector3.Lerp(posA, posB, 0.5f);
                        if (forward.sqrMagnitude > 0f && up.sqrMagnitude > 0f)
                            data.WorldRotation = Quaternion.LookRotation(forward, up);
                ...
                case IKTarget.RightFoot:
                    {
                        ...
                        var rot = HumanoidBone(HumanBodyBones.Hips).transform.rotation * VAW.VA.GetHumanoidAvatarPostRotation(HumanBodyBones.Hips) * Quaternion.Euler(90f, 90f, 0f);
                        {
        ...
        public void ChangeTargetIK(IKTarget target)
        {
            ...
            if (ikData[(int)target].enable)
            {
                ...
                if (target >= 0 && target < IKTarget.Total)
                {
                    ...
                    selectGameObjects.Add(VAW.VA.HumanoidBones[(int)IKTargetToTipBone[(int)target]]);
                }
        ...
        public bool ChangeSelectionIK()
        {
            ...
            else
            {
                ...
                if (changed)
                {
                    ...
                    VAW.VA.SelectIKTargets(selectIkTargets, null);
                }
            }
            return changed;
        }
        ...
        private static readonly (HumanBodyBones Start, HumanBodyBones Center, HumanBodyBones End)[] IKTargetHumanoidMap =
        {
            (HumanBodyBones.Neck,          HumanBodyBones.Hips,          HumanBodyBones.Head),
            (HumanBodyBones.LeftUpperArm,  HumanBodyBones.LeftLowerArm,  HumanBodyBones.LeftHand),
            (HumanBodyBones.RightUpperArm, HumanBodyBones.RightLowerArm, HumanBodyBones.RightHand),
            (HumanBodyBones.LeftUpperLeg,  HumanBodyBones.LeftLowerLeg,  HumanBodyBones.LeftFoot),
            (HumanBodyBones.RightUpperLeg, HumanBodyBones.RightLowerLeg, HumanBodyBones.RightFoot),
        ...
        {
            var idx = (int)target;
            if (idx < 0 || idx >= IKTargetHumanoidMap.Length)
                return HumanBodyBones.Hips;
            if (target == IKTarget.Head)
                return HumanoidBone(HumanBodyBones.Neck) != null ? HumanBodyBones.Neck : HumanBodyBones.Head;
            return IKTargetHumanoidMap[idx].Start;
        }
        public HumanBodyBones GetCenterHumanoidIndex(IKTarget target)
        {
            ...
            if (idx < 0 || idx >= IKTargetHumanoidMap.Length)
                return HumanBodyBones.Hips;
            return IKTargetHumanoidMap[idx].Center;
        }
        public HumanBodyBones GetEndHumanoidIndex(IKTarget target)
        {
            ...
            if (idx < 0 || idx >= IKTargetHumanoidMap.Length)
                return HumanBodyBones.Hips;
            return IKTargetHumanoidMap[idx].End;
        }
            ...
            if (VAW.VA.AnimationRigging.IsValid)
            {
                for (int i = 0; i < VAW.VA.AnimationRigging.ArRig.transform.childCount; i++)
                {
                    ...
                    foreach (var rigConstraint in rigConstraints)
                    {
                        ...
                        var typeName = mono.GetType().ToString();
                        var text = typeName;
                        {
                        ...
                        ikData[(int)target].rigConstraintGUIContent = new GUIContent(text, typeName);

                        var path = VAW.VA.GetGameObjectPath(mono.gameObject);
                        switch (target)
                        {
                            case IKTarget.Head:
        ...
        {
            return $"{AnimationRigging.AnimationRiggingRigName}_{target}";
        }
        public static GameObject GetAnimationRiggingConstraint(GameObject gameObject, IKTarget target)
        {
            ...
            if (target == IKTarget.Head)
            {
                if (!child.TryGetComponent<MultiAimConstraint>(out _))
                    return null;
            }
            else
            {
                if (!child.TryGetComponent<TwoBoneIKConstraint>(out _))
                    return null;
            }
        ...
        public static GameObject AddAnimationRiggingConstraint(GameObject gameObject, IKTarget target)
        {
            ...
            if (target == IKTarget.Head)
            {
                ...
                if (tHead != null)
                {
                    ...
                    }
                    {
                        ...
                        var max = Mathf.Max(limit.max[0], limit.max[2]);
            ...
            {
                Transform tRoot = null, tMid = null, tTip = null;
                if (target >= IKTarget.LeftHand && target <= IKTarget.RightFoot)
                {
                    tRoot = animator.GetBoneTransform(IKTargetToTwoBoneIK[(int)target, 0]);
                    tMid = animator.GetBoneTransform(IKTargetToTwoBoneIK[(int)target, 1]);
                    tTip = animator.GetBoneTransform(IKTargetToTwoBoneIK[(int)target, 2]);
                }
                if (tRoot != null && tMid != null && tTip != null)
                {
                    ...
                    string effectorTarget = String.Empty;
                    string effectorHint = String.Empty;
                        ...
                        case IKTarget.RightHand:
                            ...
                            constraint.data.maintainTargetRotationOffset = true;
                            {
                                ...
                                if (save != null)
                                {
                                    var rotation = (rootRotationInv * save.rotation);
                                    {
                                        var limitSign = uAvatar.GetLimitSign(animator.avatar, (int)HumanBodyBones.RightHand);
