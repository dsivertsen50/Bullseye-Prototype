...
namespace VeryAnimation
{
    internal class AnimationRigging
    {
        ...
        public bool IsValid => vaRigBuilder != null && VaRig != null && RigBuilder != null && RigLayer != null && ArRig != null;
        public bool IsActive => IsValid && RigBuilder.isActiveAndEnabled && RigLayer.active;
        public float Weight => IsActive ? ArRig.weight : 0f;
        ...
        public static void Delete(GameObject gameObject)
        {
            ...
            var index = rigBuilder != null && rig != null ? rigBuilder.layers.FindIndex(x => x != null && x.rig == rig) : -1;
            if (rig != null)
            {
                                                                ...
                                                                GameObject originalGameObject, Rig originalRig)
        {
            ...
            foreach (var originalRigConstraint in originalRigConstraints)
            {
                ...
                if (originalRigConstraint is BlendConstraint blendConstraint)
                {
                    var previewTransform = GetPreviewTransform(blendConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<BlendConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is ChainIKConstraint chainIKConstraint)
                {
                    var previewTransform = GetPreviewTransform(chainIKConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<ChainIKConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is DampedTransform dampedTransform)
                {
                    var previewTransform = GetPreviewTransform(dampedTransform.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<DampedTransform>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is MultiAimConstraint multiAimConstraint)
                {
                    var previewTransform = GetPreviewTransform(multiAimConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<MultiAimConstraint>(out var constraint))
                    {
                        constraint.data.constrainedObject = GetPreviewTransform(multiAimConstraint.data.constrainedObject);
                        constraint.data.worldUpObject = GetPreviewTransform(multiAimConstraint.data.worldUpObject);
                ...
                else if (originalRigConstraint is MultiParentConstraint multiParentConstraint)
                {
                    var previewTransform = GetPreviewTransform(multiParentConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<MultiParentConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is MultiPositionConstraint multiPositionConstraint)
                {
                    var previewTransform = GetPreviewTransform(multiPositionConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<MultiPositionConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is MultiReferentialConstraint multiReferentialConstraint)
                {
                    var previewTransform = GetPreviewTransform(multiReferentialConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<MultiReferentialConstraint>(out var constraint))
                    {
                        var sourceObjects = constraint.data.sourceObjects;
                ...
                else if (originalRigConstraint is MultiRotationConstraint multiRotationConstraint)
                {
                    var previewTransform = GetPreviewTransform(multiRotationConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<MultiRotationConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is OverrideTransform overrideTransform)
                {
                    var previewTransform = GetPreviewTransform(overrideTransform.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<OverrideTransform>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is TwistChainConstraint twistChainConstraint)
                {
                    var previewTransform = GetPreviewTransform(twistChainConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<TwistChainConstraint>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is TwistCorrection twistCorrection)
                {
                    var previewTransform = GetPreviewTransform(twistCorrection.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<TwistCorrection>(out var constraint))
                    {
                ...
                else if (originalRigConstraint is TwoBoneIKConstraint twoBoneIKConstraint)
                {
                    var previewTransform = GetPreviewTransform(twoBoneIKConstraint.transform);
                    if (previewTransform != null && previewTransform.TryGetComponent<TwoBoneIKConstraint>(out var constraint))
                    {
