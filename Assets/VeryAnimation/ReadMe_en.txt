...
[Update History]
Version 1.4.1
- FIX : Removed the NoAutoStaticsCleanup attribute since the warnings (UAL0010/UAL0013) no longer appear in Unity 6000.6
- FIX : Tutorial : Removed the guidance label to the Tutorial video that was displayed in Help when launching a new version, as it was not displayed correctly. The Tutorial can also be opened from the Window/Very Animation/Tutorial menu

Version 1.4.0
- ADD : Unity 6000.6 support
- ADD : Recorder : Added Animation Recorder functionality
- ADD : Timeline : Added VAAnimationTrack compatible with Additive Track
- ADD : AnimatorIK, OriginalIK : As an example, a Range Pinning function has been added that updates a specified range, which can be used for tasks such as correcting foot contact to keep the foot planted on the ground.
- FIX : Root Correction : Removed default shortcut key assignments. To restore the same behavior as before, you need to assign L in the shortcut settings.
- FIX : Minimum operating version changed to Unity 2022.3.0f1
- FIX : Tools/Create New Clip : When created in Timeline, the values of the current AnimationPlayableAsset settings are copied and applied
- FIX : Tools/Create New Clip : Modified so that editing is reopened after creating in Timeline
- FIX : Tools/Create New Clip/Mirror : Fixed the write target judgment for Generic type
- FIX : Tools/Create New Clip/Result : Removed the process to disable loop settings
- FIX : Tools/Create New Clip/Result : Fixed the output of Generics with Root node specified in Timeline
- FIX : Tools/Create New Clip/Result : Removed the initialization of AnimationClipSettings and modified it to inherit the settings of the Clip being edited
- FIX : Tools/Create New Clip/Result : In Timeline, a new Override Track is now added and the Clip is placed at the time position that was output
- FIX : Tools/Root Motion/Root Curves : Modified to match the specification that the created curves are generated based on the Root node value generated by ModelImporter, rather than the origin.
- FIX : Tools/Keyframe Reduction : Fixed an issue where negative values would sometimes be inverted when reducing curves other than Transform; modified so that reduction of curves other than Transform is now done using Position Error.
- FIX : Tools/Keyframe Reduction : Several specification changes, including a change in the name of the enabled flag.
- FIX : Tools/Create New Clip, Humanoid IK, Root Motion : Modified the process of adding all keyframes so that keys are not added if the value is the same as the previous frame.
- FIX : Tools/Bake IK : Fixed to ensure FootIK is always updated when the optional FootIK is enabled.
- FIX : Pose/Reset/Avatar Configuration-Pose : Modified to reset only Humanoid elements.
- FIX : HandPose : Fixed an issue where the muscle values of a Template saved with Save as could be empty.
- FIX : HandPose : Fixed an error that occurred when saving if the number of muscle names and values in the saved data did not match.
- FIX : HandPose, BlendShape : Fixed an error that occurred when starting the next editing session after registering a set and then switching scenes or entering play mode.
- FIX : BlendPose : Fixed an issue where the Generic root bone was excluded in Tree mode.
- FIX : BlendShape : Fixed an error that occurred when characters containing regular expression symbols were specified in the mirror name automap setting.
- FIX : AnimatorIK, OriginalIK : Fixed an issue where the Mirror button in Selection did not work.
- FIX : AnimatorIK : Fixed an error that occurred when syncing while the Animation Rigging target object had been deleted.
- FIX : AnimatorIK : Fixed an issue where the toe correction was applied twice when operating the toe-pivot rotation handle with both foot IK targets selected.
- FIX : AnimatorIK : Modified to use Spine as the Local space parent when Chest and UpperChest do not exist.
- FIX : AnimatorIK : Fixed an issue where the upper Limits value was not set correctly when creating the Head constraint for Animation Rigging.
- FIX : AnimatorIK : Fixed an error that occurred when the Animation Rigging Source Objects contained an unassigned element.
- FIX : OriginalIK : Fixed errors that occurred when loading invalid saved data.
- FIX : OriginalIK : Fixed an error that occurred when operating while a deleted IK target remained in the selection.
- FIX : OriginalIK : Improved CCD solver accuracy (model scale independent convergence threshold, non-uniform scale support for swivel axis conversion).
- FIX : OriginalIK : Unified the reference joint of the swivel guide display with the one used for swivel measurement.
- FIX : VA Tools/Reset Pose : Modified the description to match the operation.
- FIX : VA Tools/Replace Reference : Fixed an issue where replacements in Timeline Assets were not saved.
- FIX : VA Tools/Replace Reference, Remove Save Settings : Modified to save asset changes upon completion of processing.
- FIX : VA Tools/Remove Save Settings : Modified to remove components originating from nested Prefabs only on their source Prefab, and added error handling for each Prefab.
- FIX : VA Tools/Replace Reference, Remove Save Settings : Modified to limit the search target to the Assets folder, excluding Packages.
- FIX : VA Tools/Replace Reference, Remove Save Settings : Fixed an error in the scene restoration process when executed with unsaved scenes.
- FIX : VA Tools/Reset Pose : Modified to load the Prefab reference from a temporary preview instance so that the scene is not affected.
- FIX : VA Tools/Animation Rigging : Modified so that Delete All can be executed in Humanoid mode even with a non-Humanoid Animator.
- FIX : VA Tools/Animation Rigging : Modified so that Create in Copy From Other Source can be reverted with a single Undo, and Delete All no longer deselects the selection.
- FIX : VA Tools/Replace Reference, Remove Save Settings : Modified so that processing can be cancelled.
- FIX : VA Tools/Replace Reference : Modified so that it cannot be executed when the same Animation Clip is specified for both old and new.
- FIX : VA Tools/Template Pose : Added Undo support for the Template field and speeded up the apply process.
- FIX : VA Tools/Template Pose : Fixed an error that occurred when the number of BlendShape names and values in the saved data did not match.
- FIX : Pose : Fixed an error that occurred when loading if the number of BlendShape names and values in the Template's saved data did not match.
- FIX : Write Lock : Modified the write lock check for Humanoid IK Goal curves to use the target hand/foot bone instead of the root.
- FIX : VA Tools/Animation Rigging : Modified to display a warning when the Animation Rigging Package is not installed.
- FIX : VA Tools : Fixed an issue where Hierarchy changes were not reflected in the displayed target.
- FIX : SynchronizeAnimation : Fixed the operation.
- FIX : OnionSkinning : Unifying and modifying shaders for Built-in and SRP
- FIX : Edit while paused : Fixed the return process at the end of editing Animator Parameters.
- FIX : Edit while paused : Fixed an error that occurred depending on the layer state when starting editing during play mode.
- FIX : DaeExporter : Modified to use EntityId.ToULong for internal IDs in Unity 6000.4 and later.
- FIX : DaeExporter : Fixed the resolution of the texture output path and the retrieval of the Avatar skeleton information.
- FIX : Timeline : Modified so that the current time does not change when switching targets during editing.
- FIX : Timeline : Fixed positional misalignment occurring on sub-tracks with Infinite Clip enabled.
- FIX : Timeline : Fixed the Foot IK setting not being applied when using an Infinite Clip.
- FIX : Timeline : Fixed a malfunction occurring in combination with Activation Track.
- FIX : Timeline : Fixed an issue where the pose being edited was captured as the restore baseline when Preview was re-enabled after stopping recording, causing the object not to return to its original position when exiting Preview, such as when Track Offsets is Apply Scene Offsets.
- FIX : AnimationRigging : Fixed a bug where the right hand's LimitSign was referencing the left hand's information.
- FIX : AnimationRigging : Fixed an error that occurred in the Scene view skeleton drawing when a constraint referenced an object outside the hierarchy being edited.
- FIX : AnimationWindow/Filter : Modified the settings so that "Show Read-only Properties" is ON, "Enable Query Builder" is OFF, and "Search Filter" is OFF, ensuring that only curves relevant to the selection state are displayed in Filter mode.
- FIX : Fixed various behaviors when Layer Sync is enabled in the Animator Controller.
- FIX : Fixed an error that occurred in Layers mode when the Animator Controller had no layers.
- FIX : Fixed an error that occurred when a layer was added while the Layer Settings window was open.
- FIX : Fixed an error that occurred when clicking a mesh in the Scene view to select it if a SkinnedMeshRenderer had unassigned bones.
- FIX : Fixed an error that could occur when restoring the mirror bone information of the save settings.
- FIX : Fixed an issue where the pose would not return to its original state when editing was finished.
- FIX : Modified editor shaders to be hidden and not displayed in the menu.
- FIX : Fixed a bug that allowed the mixed selection of IK and bones when using the "add to selection" mode.
- FIX : Speeded up to reduce the likelihood of garbage collection.
- FIX : Addressed warnings (UAL0010/UAL0013) shown on classes with static members in Unity 6000.6 and later

Version 1.3.10
- ADD : Supports Unity 6000.5
- ADD : Supports New Hierarchy in Unity 6000.3 and later
- FIX : Humanoid : Virtual Bone : Fixed an issue where additional selection with Ctrl/Shift was lost
- FIX : EditorWindow : Options : Fixed Undo support
- FIX : Added other error handling

Version 1.3.9
- ADD : Unity6000.4 support
- FIX : Demo : Updated the settings of the old asset to fix the warning.
- FIX : BlendPose : Fixed an issue where BlendShape was not working.
- FIX : Tools/Create New Clip/Result : Fixed the created curve to be smoother
- FIX : Speeding up processing
...
- FIX : Save Settings : Fixed an issue where an error would sometimes appear when creating a new component.
...
Version 1.2.3p1
- FIX : Animation Rigging : Fixed problem when setting multiple constraints
- FIX : Preview : Constraints other than MultiAimConstraint and TwoBoneIKConstraint in Animation Rigging now work.
...
- FIX : Tools : Animation Rigging : Unified keyframe detection with Humanoid IK when generating
...
- FIX : Animation Rigging : Fixed the behavior when RigBuilder already exists.
...
Version 1.1.1p1
- ADD : Tools/Cleanup : Add BlendShape
- FIX : Tools/Cleanup : Change Eye, Jaw and Toe individually
...
- FIX : Tools : Cleanup : Correspondence of BlendShape information
...
- FIX : Correction of behavior that Global rotation operation does not reflect correctly
